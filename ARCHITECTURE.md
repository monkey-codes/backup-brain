...
Embeddings are generated by the **agent process** before calling the MCP server. The agent's `rewriteToolsForLLM()` hides embedding parameters from the LLM and injects computed vectors before each MCP call:

```
capture_thought:
  → LLM provides thought text + decisions (no embedding)
  → Agent computes embedding via OpenAI text-embedding-3-small
  → Agent injects embedding + session_id + created_by
  → MCP server receives pre-computed embedding + thought text + decisions
  → Inserts thought row with content + embedding
  → Inserts decision rows
  → Returns IDs

update_thought:
  → MCP server receives thought ID + updated text + optional pre-computed embedding
  → Updates thought row with new content + new embedding
  → Returns updated thought ID

search_thoughts:
  → LLM provides natural-language query (no embedding)
  → Agent computes query embedding via OpenAI text-embedding-3-small
  → MCP server receives pre-computed query embedding
